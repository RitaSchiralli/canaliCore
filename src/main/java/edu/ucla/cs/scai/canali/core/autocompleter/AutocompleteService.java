...
public class AutocompleteService {
	...
	public static final String CLASS = "class", ENTITY = "entity", PROPERTY = "property", DIRECT_OPERATOR = "direct_operator", INDIRECT_OPERATOR = "indirect_operator", CONSTRAINT_PREFIX = "constraint_prefix", FINAL_PUNCTUATION = "final_punctuation", LITERAL_NUMERIC = "literal_numeric", LITERAL_DATE = "literal_date", LITERAL_STRING = "literal_string", LITERAL_PERCENTAGE = "literal_percentage", QUESTION_START = "question_start", POSSESSIVE_DETERMINER = "possessive_determiner", TOPK_OPERATOR = "topk_operator", RANK_OPERATOR = "rank_operator", UNARY_OPERATOR = "unary_operator", TEXT = "text";
		...
		for (int i = 1; i <= result.length(); i++) {
			for (int j = 1; j <= typing.length(); j++) {
				if (result.charAt(i - 1) == typing.charAt(j - 1)) {
					...
					matrix[i][j] = Math.min(Math.min(matrix[i - 2][j - 2] + 1, //swap
...
*/
		ArrayList<AutocompleteObject> res = new ArrayList<>();
		//boolean queryEndsWithSpace = query.charAt(query.length() - 1) == ' ';//rita
		query = query.trim();
			...
			while (currentWordsUsed > 0) {
				...
				if (!filteredResults.isEmpty()) {
					...
					if (filteredResults.get(0).similarity >= 0.95) { //a very good token has been found
						...
						if (currentState.equals(ACCEPT_CONCEPT_STATE_S1) || currentState.equals(ACCEPT_CONSTRAINT_OR_FINAL_PUNCTUATION_STATE_S2) || currentState.equals(ACCEPT_OPERATOR_OR_DIRECT_OPERAND_STATE_S4) || currentState.equals(ACCEPT_DIRECT_OPERAND_STATE_S5) || currentState.equals(ACCEPT_INDIRECT_OPERAND_STATE_S6)) {
		...
		return null; // Modified because in the experiment we do not need the user interaction 
	...
	public ArrayList<AutocompleteObject> getSimpleAutocompleResults(String query, String lastAcceptedProperty, String[] openVariablesUri, Integer[] openVariablesPosition, String currentState, String finalPunctuation, boolean disableContextRules, boolean dateToNumber) {
		...
		case INITIAL_STATE_S0: //Only a question start can be accepted: e.g., Give me ...
			for (Token e : ontology.getTokenElements(query, null, null, null, 20, IndexedToken.QUESTION_START)) {
				...
				if (e.getText().startsWith("What has") || e.getText().startsWith("Who has")) {
					res.add(new AutocompleteObject(e.getText(), e.getText(), ACCEPT_PROPERTY_FOR_CONSTRAINT_STATE_S3, e.getText(), QUESTION_START, punctuation, null, isPrefix, null));
				} else {
		...
		case ACCEPT_CONSTRAINT_OR_FINAL_PUNCTUATION_STATE_S2:
			...
			break;
		case ACCEPT_PROPERTY_FOR_CONSTRAINT_STATE_S3:
			//being O the set of open variables, an properties a can be accepted if the intersection between O and domain(a) is not empty
			if (openVariablesUri != null && openVariablesUri.length > 0) {
			...
			break;
		case ACCEPT_OPERATOR_OR_DIRECT_OPERAND_STATE_S4:
		...
		case ACCEPT_SELF_PROPERTY_AS_DIRECT_OPERAND_STATE_S7:
			...
			for (Token e : ontology.getTokenElements(query, null, lastAcceptedPropertys, splitUris(openVariablesUri)/*
																													 * lastAcceptedFreeVariable
																													 * .
																													 * split
																													 * (
		...
		case ACCEPT_SELF_PROPERTY_AS_INDIRECT_OPERAND_STATE_S8:
			...
			for (Token e : ontology.getTokenElements(query, lastAcceptedPropertys, null, splitUris(openVariablesUri)/*
																													 * lastAcceptedFreeVariable
																													 * .
																													 * split
																													 * (
		...
		case ACCEPT_PROPERTY_FOR_RANK_STATE_S9:
			...
			results = ontology.getTokenElements(query, null, null, splitUris(openVariablesUri) /*
																								 * lastAcceptedFreeVariable
																								 * .
																								 * split
																								 * (
